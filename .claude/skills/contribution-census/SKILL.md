...
## 对照组 / 负样本分支（Negative-Sample Branch）

> 本分支用于 **T2⑤ 负样本** 协议（见 `research/METHOD_IMPROVEMENTS.md` ⑤节）。
> 目的：找"满足相似门槛（有硬成果+长期投入）但未进任何权威 AI 领军名单"的人，
> 用同一套框架打 8 维分，检验门槛特异度。

### 与 F61+ 新人入库的本质区别

| 项目 | F61+ 新人入库 | 负样本对照组 |
|---|---|---|
| 进官方名单 | 是，分配 F## ID | **否，永远不进正式名单** |
| 参与正式重评 | 是（等下一批次） | **否，结果只用于诊断** |
| 产物路径 | `profiles/` `research/facts/` `research/contributions/` | **`research/validation/negative_sample/`（物理隔离）** |
| 目的 | 补全画像覆盖 | **测门槛特异度** |
| 来源纪律 | 同 S1–S4 硬纪律 | **完全相同，不得降级** |

**严格铁律**：负样本的打分文件绝不放入 `research/scores.md` 或 `research/capability_matrix.md`。
任何合并操作都意味着循环污染——对照组一旦进入正式分数，"判别力"检验就失去意义。

### 候选人选取原则

选"近名单未入选者"，同时满足：
1. **硬成果**：至少 1 篇高被引论文（Google Scholar ≥10,000 引）或 1 个被广泛采用的系统/框架，
   或至少 1 项权威奖项（ACM Fellow/Turing/院士等）。
2. **长期投入**：在 AI/ML 领域持续 ≥10 年，非"一篇爆款后消失"。
3. **未入选**：未出现在任何主要权威 AI 领军名单（TIME100 AI、Forbes AI 50 等）里。
4. **可核验性**：事实来源 S1/S2 可以查实，减少"可能是真实但无法证实"的情形。

覆盖类型要尽量多元（正式名单系统性低估的那几类）：
纯理论数学家、关键但低调的工程师/架构师、开源社区贡献者、二线机构 PI 等。

### 负样本工作流程（按顺序执行）

**NS-Step 0 — 候选确认**
- 明确候选人姓名与"未入名单"的依据（已查过哪些名单、确认不在）。
- 说明"够格当对照"的理由（接近门槛的硬成果方向）。

**NS-Step 1 — 轻量普查（同 Step 1–2 的八类穷举，但可精简）**
- 复制 `research/contributions/TEMPLATE.md` 到 `research/validation/negative_sample/NS_<name>.md`。
- 对八类维度逐类检索，每条贡献带来源 URL + S 级 + 影响数字 + as-of 日期。
- 同样遵守来源硬度规则：≥2 分建议必须有 S1/S2 支撑。

**NS-Step 2 — 用同一 rubric 打 8 维分**
- 按 `research/capability_matrix.md` 折算规则（命中条数×显著度）给出 D1–D8 的 0–3 分。
- 同样列出命中的规范能力 ID 与支撑事实，保持可下钻性。
- **结果只写在本文件**，绝不触碰正式分数文件。

**NS-Step 3 — 写结论段（"分不开"vs"系统性低分"读法）**
- 与 60 人分数分布比较（肉眼比对 `research/scores.md` 表格即可）。
- 给出初步判断：该人落在 60 人分布的哪个区间？是否与某个原型重叠？
- 标注事实存疑处（S3/S4 来源未升级的条目）。

**NS-Step 4 — 更新 README**
- 在 `research/validation/negative_sample/README.md` 的候选人表格里登记该人的 8 维分与结论。

### 自检（同正式 census，加一条）
- 确认**未修改** `research/scores.md`、`research/capability_matrix.md`、任何 `profiles/` 或 `research/facts/` 文件。
- 确认产物仅在 `research/validation/negative_sample/` 目录下。
- 每条贡献至少 1 个 S1/S2 来源；指标带口径与 as-of 日期。

### 完成汇报（负样本版）
向用户报告：候选人名、8 维分数向量、来源硬度分布、与 60 人分布的比较结论
（"分不开"→门槛特异度低，"系统性低分"→有判别信息），以及待核实事实的清单。
